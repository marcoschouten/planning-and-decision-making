...
def collisionFree(X, V, ws_model, sampled_V):
    # check about no collision with the world

    tau = 1
    n_steps = 10
    # check about no collision with other moving obstacles
    vj = np.array(V[1]) # velocity of non collaborating moving obstacle
    vi = np.array(sampled_V) # velocity of ourself
    pj =  np.array(X[1]) # pos of obstacle
    pi =  np.array(X[0]) # pos of us
    rj = ws_model['robot_radius']+ 0.1 # radius of obstacle
    ri = ws_model['robot_radius'] + 0.1  # radius of obstacle
...
def VO_update(X, V_des, V, ws_model):
    # get data

    if ( np.linalg.norm(np.array(X[0])-np.array(X[1])) <  2): # we are in range of a collision
        # fill an array of N collision free velocities
        num_cf_V = 3
        cf_V = []
        count_cv_V = 0
        while count_cv_V < num_cf_V:
            # sample velocity
            sampled_V = sampleVel(X, max_vel=1, world=None)
            # check if velocity leads to collision
            if collisionFree(X, V, ws_model, sampled_V):
        ...
        for i in range(0, num_cf_V):
            cosine_similarity[i] = spatial.distance.cosine(V_des[0], cf_V[i], w=None)
...
def compute_V_des_marco(X, goal, V_max):
    V_des = []
    a = (-X[0]+goal[0]) / np.linalg.norm(X[0]-goal[0]) * V_max[0]
    b = (-X[1] + goal[1]) / np.linalg.norm(X[1] - goal[1]) * V_max[1]
    V_des.append(a)
    V_des.append(b)
    return V_des
