...
class dwa_planner:
    ...
    def sample_vel(self):
        ...
        while i < self.samp_num:
            ...
            sampled_point = SphereSampler(
                self.state.vel, 0.1*self.max_acc*self.dt, num=1)
    ...
    def vel_selection(self, sampled_vel, clearance):
        ...
        way_heading = (self.waypoints[segnum+1]-pos) / \
            LA.norm(self.waypoints[segnum+1]-pos)
        print(self.waypoints[segnum+1])
        print(pos)
        ...
        cost = 10/clearance+1*ang_cur_goal+2 * \
            ang_cur_way+3/LA.norm(sampled_vel, axis=1)
